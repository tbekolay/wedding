...
def add(folder, index=1):
    """Adds the given folder to the python path.
    ...
    folder : str
        Path to the folder containing packages, relative to ``os.getcwd()``
    index : int
        Where in ``sys.path`` to insert the vendor packages. By default
        this is set to 1. It is inadvisable to set it to 0 as it will
        override any modules in the current working directory.
    """
